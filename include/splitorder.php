...
if (!class_exists('wosc_default_class')) {

    class wosc_default_class {
...
//                 create sub order //
        ...
        function sunarc_change_order_received_text($str, $order) {
            ...
            $str .= $this->wosc_change_order_received_text($order_id);
            return $str;
        }

        function wosc_change_order_received_text($order_id, $deleted = true) {
		    include('mail.php');
            ...
            if (!empty($posts_array)) {
                ...
                <section class="woocommerce-order-details child_orderss">
                    <h2 class="woocommerce-order-details-title"><?php echo $wos_cart_notices['co_heading'] ? $wos_cart_notices['co_heading'] : 'Split Order' . (count($posts_array) > 1 ? 's' : ''); ?></h2>
                    <?php
	...
	public function change_formatted_meta_data($orders) {
        ...
        public function sunarc_thank_you_page_template($template, $template_name, $template_path) {
            if ('checkout/thankyou.php' == $template_name) {
                $template = wosc_sunarc_plugin_dir . '/include/checkout/thankyou.php';
            }
            return $template;
